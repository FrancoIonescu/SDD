...
HashTable initHashTable(int dim) {
	...
	hashTable.vector = (Nod**)malloc(dim * sizeof(Nod*));
	for (int i = 0; i < dim; i++)
	{
...
// serie % dim    Va da valoare intre 0 si dim - 1
// strlen(sir ex: producator) % dim
// pot trimite prin valoare ht si nu prin adresa pt ca adresa se face in initMasina, si nu o modific, ci doar adaug la 
