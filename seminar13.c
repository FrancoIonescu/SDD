...
void parcurgereInAdancime(NodPrincipal* graf, int idStart) {
	int dimensiune = numarNoduri(graf);
	int* vectorNoduriVizitate = (int*)malloc(dimensiune * sizeof(int));
...
// TO DO CALCUL NUMAR NODURI DIN GRAF
...
void main() {
	...
	int dimensiune = numarNoduri(graf);
	printf("\n\n Numarul total de noduri din graf: %d", dimensiune);
